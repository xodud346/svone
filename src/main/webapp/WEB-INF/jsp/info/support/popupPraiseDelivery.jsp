...
%><%
String BU_NAME = getCheckReqXSSTop(request, "BU_NAME", "");	//물류센터
String KS_NAME = getCheckReqXSSTop(request, "KS_NAME", "");	//배송기사
%>
<head>
<meta charset="utf-8" />
...
<script type="text/javascript" src="<c:out value="${cdnDomain}" />/pc/js/search/admin_ark/js/ark_center.js"></script>
<script type="text/javascript" src="<c:out value="${cdnDomain}" />/pc/js/search/admin_ark/js/ark_delivery.js"></script>
...
<div id="popContainer"><!-- popContainer -->
	<div class="cheer-search-hd">
    	<h1 class="title">물류센터/배송기사 검색</h1>
 
        <div class="search-form"><!-- staff-search -->
            <p>
                <label for="BU_NAME">물류센터</label>
                <input type="text" id="BU_NAME" class="text" value="<%=BU_NAME %>" placeholder="센터명을 입력해주세요" />
            </p>
            <div id="centerSearchList" class="search-list-wrap">
            	<ul id="ark_content_list_center">
            	</ul>
            </div>
            <p class="mt10">
                <label for="KS_NAME">배송기사</label>
                <input type="text" id="KS_NAME" class="text" value="<%=KS_NAME%>" placeholder="배송기사명을 입력해주세요" />
            </p>
            <div id="delSearchList" class="search-list-wrap">
            	<ul id="ark_content_list_del">
            	</ul>
            </div>
	...
	<div id="popWrapper" class="cheer-search"><!-- popWrapper -->
        ...
        <p class="mt20">
            <span class="large14 primary">그래도 검색결과에 찾으시는 배송기사가 없으신가요?</span>
        </p>
...
//	alert("goReqPage");
	...
	$.ajax({
		type		: "POST",
		url			: "/info/support/deliveryListAjax.do",
		dataType	: "json",
