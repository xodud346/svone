		...
		<div id="contents"><!-- contents -->
			...
			<div class="container">
				...
				<form id="frm" name="frm">
					<input type="hidden" id="autMstIdx" name="autMstIdx" value="<c:out value="${commandMap.autMstIdx }"/>"/>
					<table cellspacing="0" class="table-row table-a"><!-- table -->
						...
						<tbody>
							<tr><!-- row -->
								...
								<td>
									<input type="text" class="text large" name="autMstNm" id="autMstNm" maxlength="40" style="width:80%"value='<c:out value="${row.autMstNm}" />'/>
								</td>
							</tr>
						</tbody>
					</table><!-- // table -->
											...
											<c:forEach var="row" items="${menuList}" varStatus="i">
												<tr>
													<th>
														<span>
															<label>
																<input type="checkbox" name="admMnuIdx1" id="admMnuIdx1_<c:out value="${i.index}"/>" class="checkbox admin_mnu_depth1"
																value='<c:out value="${row.ADM_MNU_IDX }"/>' <c:if test="${row.checked eq 'true'}">checked</c:if>/>
													...
													<td>
														...
														<c:forEach var="subMenu1" items="${row.childMenuList }" varStatus="j">
															<input type="checkbox" name="admMnuIdx2" class="checkbox admin_mnu_depth2"
															value='<c:out value="${subMenu1.ADM_MNU_IDX}"/>'data-parent="<c:out value="${i.index}"/>"
															data-target="<c:out value="${i.index}"/>-<c:out value="${j.index}"/>"
															<c:if test="${subMenu1.checked eq 'true'}">checked</c:if> />
									...
									<c:choose>
										...
										<c:otherwise>
											<c:forEach var="memberlist" items="${memberlist}" varStatus="i">
												<tr>
													<td><input type="checkbox" name="admMstIds" class="checkbox checkRow" /></td>
													<td>${memberlist.orgNm }</td>
													<td>${memberlist.admMstErpNo }</td>
													<td>${memberlist.admMstNm }
													<input type='hidden' name='admMstId'  value='${memberlist.admMstId}' />
													</td>
												</tr>
											</c:forEach>
										</c:otherwise>
				...
				<div class="section-button"><!-- section-button -->
					<div class="wrap text-center">
						...
						<c:if test="${isSuperAdmin}">
							<a href="javascript:void(0);" onClick="fnDelAuth();" class="button large"><span>권한삭제</span></a>
						</c:if>
						<a href="javascript:void(0)" onClick="fnGoList();" class="button large"><span>취소</span></a>
...
$(function() {
	...
	<!-- 목록 버튼 클릭 이벤트 -->
	fnGoList = function () {
		$frm.attr({"action":"/am/site/authManagerList.do", "target":"_self", "method":"post"}).submit();
	};
	...
	$("#goRegist").on("click", function () {
		...
		if($frm.valid()){
			...
			if (confirm( "저장 하시겠습니까?")) {
				$frm.attr({"action":"/am/site/authManagerModify.do", "target":"_self", "method":"post"}).submit();
			}
		}
	});
	...
	fnDelAuth = function() {
		if($frm.valid()){
			if (confirm( "삭제 하시겠습니까?")) {
				$frm.attr({"action":"/am/site/authManagerDelete.do", "target":"_self", "method":"post"}).submit();
			}
		}
	}
	...
	fnResultReg = function(params){
		...
		po += '<tr>';
		po += 	'<td>';
		po += 		'<input type="checkbox" name="admMstIds" class="checkbox checkRow" value="<c:out value="'+params.admMstId+'"/>"/>';
		po += 	'</td>';
		po += 	'<td>';
		po +=		+ params.orgNm ;
		po += 	'</td>';
	...
	<!-- 담당자 삭제 버튼 -->
	memberDel = function (){
		if($("input[name=admMstIds]:checked").length < 1) {
			alert("선택된 데이터가 없습니다.")
			return false;
		}
		$("input[name=admMstIds]:checked").each(function(idx, data) {
