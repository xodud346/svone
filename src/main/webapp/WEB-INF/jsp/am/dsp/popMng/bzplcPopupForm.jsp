    ...
    <div class="modal">
        <div class="modal-wrapper">
            <div class="modal-inner">
					...
					<tbody id="_selectedData">
						<c:if test="${not empty selList}">
					    	<c:forEach var="list" items="${selList}" varStatus="status">
								<tr>
									<td><input type="checkbox" id="SEL_BZPLC_ID_${list.BZPLC_ID}" name="SEL_BZPLC_IDs" class="checkbox checkRow" data-id="${list.BZPLC_ID}" data-nm="${list.BZPLC_NM}" data-no="${list.BZMN_REG_NO}"/></td>
									<td>${list.BZPLC_ID}</td>
									<td>${list.BZPLC_NM}</td>
									<td>${list.BZMN_REG_NO}</td>
								</tr>	
					    	</c:forEach>
					    </c:if>
					</tbody>
				...
				<div class="grid section-button-search"><!-- 검색 하단 버튼 -->
                    <a href="javascript:bzplcDel();" id="bzplcDel" class="button small"><span>삭제</span></a>
...
$(function(){
	...
	// 검색
    search = function() {
		$("#popList").load("/am/dsp/popMng/bzplcPopupList.do", $frm.serialize(), function(){
    ...
    bzplcSel = function () {
    	...
    	window.opener.fnResultBzplc(params);
		self.close();
    }
