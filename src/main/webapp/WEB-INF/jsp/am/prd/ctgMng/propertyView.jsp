...
<div id="container">
	...
	<div id="wrapper">
		...
		<div id="contents">
			<div class="container">
				<h2 class="title"><span>SSP속성 목록</span></h2>
				...
				<form id="searchForm" name="searchForm">
					...
					<input type="hidden" id="prdSprCd" name="prdSprCd" value="10"/>
				...
				<div class="grid section-button-list">
					...
					<div class="col-1-2 text-right">
						...
						<select class="select" name="selSort" id="selSort">
							<option value="ATTR_CD_ASC">SSP속성명↓</option>
							<option value="ATTR_CD_DESC">SSP속성명↑</option>
							<option value="ATTR_NM_ASC">SSP속성코드↓</option>
							<option value="ATTR_NM_DESC">SSP속성코드↑</option>
							<option value="ATTR_CNT_ASC">SSP속성사용상품군수↓</option>
							<option value="ATTR_CNT_DESC">SSP속성사용상품군수↑</option>
				...
				<!-- 상품속성 리스트 영역 -->
				<div id="propertyList">
					<%@ include file="/WEB-INF/jsp/am/prd/ctgMng/propertyList.jsp" %>
				</div>
				<!-- //상품속성 리스트 영역 -->
			</div>
		</div>
...
$(function(){
	...
	$("#dateSet").change(function(){
		...
		if("DAY" == dateType) {	// 일
			fnDateSet('searchStartDate', 'searchEndDate', 0, 0, -dateVal, 0, 0, 0,  '-');
		} else if("WEEK" == dateType) {	// 월
			fnDateSet('searchStartDate', 'searchEndDate', 0, -dateVal, 0, 0, 0, 0,  '-');
		} else if("YEAR" == dateType) {	// 년
			fnDateSet('searchStartDate', 'searchEndDate', -dateVal, 0, 0, 0, 0, 0,  '-');
		} else {
			$("#searchStartDate").val("");
			$("#searchEndDate").val("");
		}
	...
	// 검색
	search = function() {
		$("#propertyList").load("/am/prd/ctgMng/propertyList.do", $searchForm.serialize());
	};
	...
	goForm = function (attrCd) {
		...
		$searchForm.attr({"action":"/am/prd/ctgMng/propertyForm.do", "target":"popup_propertEditPopup", "method":"post"}).submit();
	};
	...
	goExcelDown = function () {
		...
		$searchForm.attr({"action": "/am/prd/ctgMng/propertyListExcelDownload.do", "target":"_self", "method":"post"}).submit(); 
	};
