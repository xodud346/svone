...
<div id="container">
	...
	<div id="wrapper">
		...
		<div id="contents">
			<div id="container">
				<h2 class="title"><span>등록상품 목록</span></h2>
					...
					<table cellspacing="0" class="table-row table-a" style="margin-bottom: 0;">
						...
						<tbody>
							<tr>
								...
								<td colspan="3">
									<input type="text" class="text-large" id="searchId" name="searchId" placeholder="상품ID, 통합상품ID, 그룹ID를 직접 입력하여 검색"/>
							...
							<tr>
								...
								<td colspan="3">
									<select class="select" id="selectCategory" name="selectCategory">
										<option value="" selected>SSP카테고리</option>
										<option value="smro">S-MRO카테고리</option>
									</select>
									<input type="text" class="text" id="searchPrdClsCd" name="searchPrdClsCd" />
									<input type="text" class="text" id="searchPrdClsNm" name="searchPrdClsNm" />
									<a href="javascript:openCategoryPopup();" class="icon search2"></a>
								</td>
								<th><span>대표규격</span></th>
								<td>
									<input type="text" class="text" id="searchSpec" name="searchSpec" />
								</td>
							</tr>
							<tr>
								<th><span>일자</span></th>
								...
								<th><span>제조원</span></th>
								<td>
									<input type="text" class="text" id="searchMnfrNo" name="searchMnfrNo" />
									<input type="text" class="text" id="searchMnfrNm" name="searchMnfrNm" />
									<a href="javascript:openManufacturerPopup();" class="icon search2"></a>
								</td>
							</tr>
							<tr>
								<th><span>키워드</span></th>
							...
							<tr>
								...
								<td colspan="3">
									...
									<input type="text" class="text" id="searchManagerId" name="searchManagerId" />
									<input type="text" class="text" id="searchManagerNm" name="searchManagerNm" />
							...
							<tr>
								...
								<td>
									<select class="select" id="searchPrice" name="searchPrice">
										...
										<option value="2">시황자재</option>
										<option value="3">미끼상품</option>
									</select>
								</td>
							</tr>
							<tr>
				...
				<form id="listForm">
					...
					<div class="grid section-button-list">
						<div class="wrap text-right">
							<a href="javascript:;" class="button small primary"><span>일괄수정</span></a>
							...
							<select class="select medium" id="selectSort" name="selectSort">
								...
								<option value="smroDtmDesc">S-MRO등록일↓</option>
								<option value="smroDtmAsc">S-MRO등록일↑</option>
								<option value="prdIdDesc">상품ID↓</option>
								<option value="prdIdAsc">상품ID↑</option>
								<option value="prdNmDesc">상품명↓</option>
								<option value="prdNmAsc">상품명↑</option>
								<option value="categoryIdDesc">카테고리ID↓</option>
								<option value="categoryIdAsc">카테고리ID↑</option>
							</select>
					...
					<div id="productList">
						<%@ include file="/WEB-INF/jsp/am/prd/prdMng/registProductList.jsp" %>
					</div>
				</form>
			</div>
		</div>
...
$(function(){
	...
	goSearch = function(){
		...
		$listForm.find("input[name='searchPrdNm']").val($("#searchPrdNm").val());
		$listForm.find("input[name='searchFullname']").val($("#searchFullname").val());
		$listForm.find("input[name='selectCategory']").val($("#selectCategory").val());
		$listForm.find("input[name='searchPrdClsCd']").val($("#searchPrdClsCd").val());
		$listForm.find("input[name='searchSpec']").val($("#searchSpec").val());
		...
		$listForm.find("input[name='searchMnfrNo']").val($("#searchMnfrNo").val());
		$listForm.find("input[name='selectKeyword']").val($("#selectKeyword").val());
		$listForm.find("input[name='searchKeyword']").val($("#searchKeyword").val());
		$listForm.find("input[name='searchIntgPrdSpr']").val($("#searchIntgPrdSpr").val());
		$listForm.find("input[name='selectManager']").val($("#selectManager").val());
		$listForm.find("input[name='searchManagerId']").val($("#searchManagerId").val());
		$listForm.find("input[name='searchSspOnlySpr']").val($("#searchSspOnlySpr").val());
		$listForm.find("input[name='searchUseYn']").val($("#searchUseYn").val());
		$listForm.find("input[name='searchUseStatsCd']").val($("#searchUseStatsCd").val());
		$listForm.find("input[name='searchPrice']").val($("#searchPrice").val());
		$listForm.find("input[name='searchBadge']").val($("#searchBadge").val());
		$listForm.find("input[name='searchDisplay']").val($("#searchDisplay").val());

		$("#productList").load("<c:out value="${serverDomain}" />/am/prd/prdMng/registProductList.do", $searchForm.serialize());
	}
