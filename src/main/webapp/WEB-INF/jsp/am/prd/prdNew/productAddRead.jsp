		...
		<div id="contents">
			<div id="container">
				...
				<form id="searchForm">
					...
					<input type="hidden" id="inputMnfrNo" name="inputMnfrNo" value="${info.mnfrNo }" />
					<input type="hidden" id="inputAttrVal" name="inputAttrVal" value="" />
					<input type="hidden" id="inputUseYn" name="inputUseYn" />
					...
					<table cellspacing="0" class="table-row table-a">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<c:if test="${info.sspOnlySpr eq 'P'}">공용</c:if>
									<c:if test="${info.sspOnlySpr eq 'E'}">전용</c:if>
								</td>
							</tr>
							<tr>
								...
								<th><span>제조원</span></th>
								<td>
									<c:if test="${info.mnfrNo ne null }">
										(${info.mnfrNo })${info.mnfrNm }
									</c:if>
								</td>
								<th><span>원산지</span></th>
								<td>
									<c:if test="${info.orgplcCd ne null }">
							...
							<tr>
								...
								<td>
									${fn:substring(info.intgRegDtm, 0, 19) }
								</td>
								<th><span>SSP등록일</span></th>
								<td>
									${fn:substring(info.sspRegDtm, 0, 19) }
								</td>
								<th><span></span></th>
								<td></td>
							</tr>
							...
							<table cellspacing="0" class="table-row table-a">
								...
								<tbody>
									<tr>
										...
										<td>
											(${info.prdClsCd })${info.prdClsLvlNm }
										</td>
									</tr>
									<tr>
										<th><span>속성</span></th>
									...
									<tr>
										...
										<td>
											<select class="select xlarge" id="inputPrdClsCd" name="inputPrdClsCd">
												<c:if test="${fn:length(sspCategoryList) eq 1 }">
													<c:forEach items="${sspCategoryList }" var="item" varStatus="status">
														<option value="${item.prdClsCd }">(${item.prdClsCd })${item.prdClsLvlNm }</option>
													</c:forEach>
												</c:if>
												<c:if test="${fn:length(sspCategoryList) ne 1 }">
									...
									<tr>
										...
										<td>
											<table cellspacing="0" class="table-col table-a">
												...
												<tbody id="sspSpec">
													<c:if test="${not empty sspSpecList }">
														<c:forEach items="${sspSpecList }" var="item" varStatus="status">
															<tr>
																<td>${item.attrNm }<c:if test="${item.mandYn eq 'Y' }"><i class="require"><em>필수입력</em></i></c:if></td>
																<td>
																	<input type="text" class="text xlarge" id="inputPrvl" name="inputPrvl" <c:if test="${item.mandYn eq 'Y' }">data-require="Y"</c:if> />
							...
							<table cellspacing="0" class="table-row table-a">
								...
								<tbody>
									<tr>
										...
										<td colspan="2">
											<input type="text" class="text xlarge" id="inputBrndNm" name="inputBrndNm" />
										</td>
									</tr>
									<tr>
										<th><span>해시Tag</span></th>
										<td colspan="2">
											<input type="text" class="text xlarge" id="inputHashTag" name="inputHashTag" />
										</td>
									</tr>
									<tr>
										...
										<td>
											<select class="select" id="inputPrdBgCd" name="inputPrdBgCd" >
												...
												<c:forEach items="${badgeList }" var="item" varStatus="status">
													<option value="${item.prdBgCd }" data-fileCts="${item.fileCts }" data-fileStorPath="${item.fileStorPath }" data-storFileNm="${item.storFileNm }">${item.prdBgNm }</option>
												</c:forEach>
											</select>
											<br><br>
					...
					<table cellspacing="0" class="table-row table-a">
						...
						<tbody>
							<tr>
								<td>
									<textarea class="textarea" id="inputPrdDtlDesc" name="inputPrdDtlDesc" style="height: 500px; width:80%;"></textarea>
								</td>
							</tr>
						</tbody>
					</table>
					...
					<table cellspacing="0" class="table-row table-a">
						...
						<tbody>
							<tr>
								...
								<td>
									<c:if test="${info.chrMdId ne null }">
					...
					<div class="tab-list" id="menuViewTab">
						<ul>
							<li class="in"><a href="#none" data-tab_no="tab1">화학물질정보</a></li>
						...
						<table cellspacing="0" class="table-row table-a">
							...
							<thead>
								<tr>
									...
									<th colspan="5" style="text-align:center;"><span>유해화학물질여부</span></th>
					...
					<div id="tab3" style="display:none;">
						...
						<table class="table-row table-a">
							...
							<tbody>
								<tr>
									<th><span>품목</span></th>
									<td>
										<input type="text" class="text" id="noticeInfo" />
										<input type="hidden" id="inputNotiId" name="inputNotiId" value="" />
						...
						<div id="productNoticeDetail" style="display:none;">
				...
				<div class="section-button-list">
					<div class="wrap text-center">
						<a href="javascript:goList();" class="button"><span>취소</span></a>
						<a href="javascript:goRegist();" class="button primary"><span>등록</span></a>
					</div>
				</div>
			</div>
		</div>
...
$(function(){
	...
	goList = function(){
		$searchForm.attr({"action":"<c:out value="${serverDomain}" />/am/prd/prdNew/productAddView.do", "target":"_self", "method":"post"}).submit();
	}

	goRegist = function(){
		var attrVal = "";
		var miss = 0;
		var prdClsCd = $("select[name='inputPrdClsCd']").val();

		if("${info.useStatsCd}" == '10' || "${info.useStatsCd}" == '20'){
			$("#inputUseYn").val("N");
		}else{
			$("#inputUseYn").val("Y");
		}

		if(prdClsCd == "" || prdClsCd == null){
			alert("등록 불가 : 카테고리 미선택");
			return;
		}
		...
		$searchForm.attr({"action":"<c:out value="${serverDomain}" />/am/prd/prdNew/productAddCreate.do", "target":"_self", "method":"post"}).submit();
	}
});
</script>
</body>
