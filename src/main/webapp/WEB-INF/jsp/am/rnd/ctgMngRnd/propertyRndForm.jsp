...
<head>
	<c:choose>
		<c:when test="${not empty propertyInfo}">
			<title>R&D속성 상세</title>
		</c:when>
		<c:otherwise>
			<title>R&D속성 추가</title>
		</c:otherwise>
	</c:choose>
</head>
	...
	<div class="modal">
		<div class="modal-wrapper">
			<div class="modal-inner">
				<c:choose>
					<c:when test="${not empty propertyInfo}">
						<h3 class="title"><span>R&D속성 상세</span></h3>
					</c:when>
					<c:otherwise>
						<h3 class="title"><span>R&D속성 추가</span></h3>
					</c:otherwise>
				</c:choose>
...
$(function(){
	...
	attrNmValidation = function () {
		...
		$.ajax({
			type    : "POST",
			url     : "/am/rnd/ctgMngRnd/propertyRndChk.do",
			data    : $frm.serialize(),
			dataType : "json",
			async 	: false,
	...
	goSave = function () {
		...
		if (confirm(confirmMsg)) {
			 $.ajax({
				type    : "POST",
				url     : "/am/rnd/ctgMngRnd/propertyRndSave.do",
				data    :  $frm.serialize(),
				dataType : "json",
				success : function(data) {
					alert(data.resultMsg);
