		...
		<div id="contents">
			<div id="container">
				...
				<form id="searchForm" name="searchForm" enctype="multipart/form-data">
					...
					<input type="hidden" id="oldIddtId" name="oldIddtId" value="${iddt.iddtId }" />
					<input type="hidden" id="iddtChangeYn" name="iddtChangeYn" value="" />
					<input type="hidden" id="oldPrdId" name="oldPrdId" value="${prd.prdId }" />
					<input type="hidden" id="prdChangeYn" name="prdChangeYn" value="" />
					<input type="hidden" id="searchCnt" name="searchCnt" value="3" />
					<input type="hidden" id="atflId" name="atflId" value="${result.atflId }" />
					<input type="hidden" id="prePkgId" name="prePkgId" value="${result.prePkgId }" />
					<input type="hidden" id="delFileSeq" name="delFileSeq" value="" />
					<h3 class="title"><span>프리패키지 수정</span></h3>
					...
					<table cellspacing="0" class="table-row table-a">
						...
						<tbody>
							<tr>
								...
								<td colspan="4">${result.prePkgId }</td>
							</tr>
							<tr>
								<th><span>사용 상태</span></th>
								<td colspan="4">
									<input type="radio" class="radio" id="useYn1" name="useYn" value="Y" <c:if test="${result.useYn eq 'Y' }">checked="checked"</c:if> />
							...
							<tr>
								...
								<td colspan="4">
									<input type="radio" class="radio" id="allIncYn1" name="allIncYn" value="Y" <c:if test="${result.allIncYn eq 'Y' }">checked="checked"</c:if> />
									<label for="allIncYn1">포함</label>
									<input type="radio" class="radio" id="allIncYn2" name="allIncYn" value="N" <c:if test="${result.allIncYn ne 'Y' }">checked="checked"</c:if> />
									<label for="allIncYn2">미포함</label>
								</td>
							</tr>
							<tr>
								<th><span>패키지명<i class="require"><em>필수입력</em></i></span></th>
								<td colspan="4">
									<input type="text" class="text-xlarge essential" id="prePkgNm" name="prePkgNm" value="${result.prePkgNm }" />
								</td>
							</tr>
							<tr>
								...
								<td colspan="4">
									...
									<input type="text" class="text-large" id="iddtNm" name="iddtNm" value="${iddt.iddtNm }" readonly="readonly"/>
							...
							<tr>
								...
								<td colspan="4">
									<input type="text" class="text-large" id="prdId" name="prdId" value="${prd.prdId }" readonly="readonly"/>
							...
							<tr>
								...
								<td colspan="4">
									<input type="text" class="text-small essential" id="srwr" name="srwr" value="${result.srwr }" />
								</td>
							</tr>
							<c:forEach items="${imageList }" var="image" varStatus="i">
								<c:if test="${image.fileCdId eq 'P' }">
									<tr>
										<th><span>PC 이미지<i class="require"><em>필수입력</em></i></span></th>
										<td colspan="4">
											<input type="file" class="file hide" name="file" data-view="fileView1" data-type="P" data-target-file-cts="fileCts1" accept="image/png" data-seq="${image.fileSeq}" />
											...
											<div id="fileView1" class="mt10" style="border: solid 1px #d0d0d0; min-height: 30px;">
												<ul style="padding: 5px;">
													<li data-file-seq="${image.fileSeq }">
														...
														<span class="file-list-title" id="PFileNm"><c:out value="${image.orignlFileNm }" />
								...
								<c:if test="${image.fileCdId eq 'M' }">
									<tr>
										<th><span>Mobile 이미지<i class="require"><em>필수입력</em></i></span></th>
										<td colspan="4">
											<input type="file" class="file hide" name="file" data-view="fileView2" data-type="M" data-target-file-cts="fileCts2" accept="image/png" data-seq="${image.fileSeq}" />
											<a href="javascript:;" onclick="$(this).prev().click();" class="button small"><span>파일첨부</span></a>
											<span>용량 : 10M 미만 / 형식 : PNG (최적 사이즈 : 0,000 * 0,000)</span>
											<br>
											<div id="fileView2" class="mt10" style="border: solid 1px #d0d0d0; min-height: 30px;">
												<ul style="padding: 5px;">
													<li data-file-seq="${image.fileSeq }">
														<i class="icon-file" aria-hidden="true"></i>
														<span class="file-list-title" id="MFileNm"><c:out value="${image.orignlFileNm }" />
															<input type="hidden" name="fileSeq" value="${image.fileSeq }"/>
															<input type="hidden" name="fileCdId" value="${image.fileCdId }"/>
							...
							<tr>
								...
								<td colspan="4">
									<textarea class="textarea xlarge" rows="7" cols="" name="prePkgDesc">${result.prePkgDesc }</textarea>
								</td>
							</tr>
						</tbody>
					</table>
...
$(function(){
	...
	updateSubmit = function(){
	    ...
	    if($("#oldIddtId").val() != $("#iddtId").val()){
	    	$("#iddtChangeYn").val("Y");
	    } else {
	    	$("#iddtChangeYn").val("N");
	    }
	    
	    if($("#oldPrdId").val() != $("#prdId").val()){
	    	$("#prdChangeYn").val("Y");
	    } else {
	    	$("#prdChangeYn").val("N");
	    }
		...
		if(confirm("수정 하시겠습니까?")){
			...
			$searchForm.attr({"action":"<c:out value="${serverDomain}" />/am/prm/ppgMng/prepackageUpdate.do", "target":"_self", "method":"post"}).submit();
		}
	}
