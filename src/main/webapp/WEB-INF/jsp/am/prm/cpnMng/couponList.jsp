	                        ...
	                        <tr>
	                            ...
	                            <td>
	                                <select id="searchCpnMstGbn" name="searchCpnMstGbn" class="select" title="쿠폰종류">
										...
										<option value="F" <c:if test="${commandMap.searchCpnMstGbn eq 'F' }">selected="selected"</c:if>>배송비</option>
					                </select>
	                            </td>
	                            <th><span>상태</span></th>
	                            <td>
	                                ...
	                                <label for="stateC">비 활성화</label>
	                            </td>
	                            <th><span>발급방식</span></th>
	                            <td>
	                                <select id="searchCpnMstIsuGbn" name="searchCpnMstIsuGbn" class="select" title="발급방식">
										<option value="">전체</option>
										<option value="D" <c:if test="${commandMap.searchCpnMstIsuGbn eq 'D' }">selected="selected"</c:if>>회원가입</option>
										<option value="B" <c:if test="${commandMap.searchCpnMstIsuGbn eq 'B' }">selected="selected"</c:if>>대상자 지정</option>
										<option value="C" <c:if test="${commandMap.searchCpnMstIsuGbn eq 'C' }">selected="selected"</c:if>>다운로드</option>
					                </select>
	                            </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
	                            ...
	                            <td colspan="5">
	                            	<select id="searchCpnMstSeDt" name="searchCpnMstSeDt" class="select" title="기간검색">
										...
										<option value="B" <c:if test="${commandMap.searchCpnMstSeDt eq 'B' }">selected="selected"</c:if>>발행일</option>
					                </select>
		                    	...
		                    	<c:when test="${not empty list }">
									...
									<c:forEach var="row" items="${list }" varStatus="i">
										<tr>
											...
											<td>
												<c:out value="${row.CPN_MST_YN eq 'Y' ? '활성화' : '비 활성화' }"/>
		                               		</td>		                               		
											<td>
												<c:choose>
													<c:when test="${row.CPN_MST_GBN eq 'A'}">일반
													</c:when>
													<c:when test="${row.CPN_MST_GBN eq 'F'}">배송비
													</c:when>
													<c:otherwise>전체
													</c:otherwise>
												</c:choose>
											...
											<td>
								                <c:choose>
													<c:when test="${row.CPN_MST_ISU_GBN eq 'B'}">대상자 지정 발행
													</c:when>
													<c:when test="${row.CPN_MST_ISU_GBN eq 'C'}">다운로드발급
													</c:when>
													<c:when test="${row.CPN_MST_ISU_GBN eq 'D'}">회원가입
													</c:when>
													<c:otherwise>${row.CPN_MST_ISU_GBN}
													</c:otherwise>
												</c:choose>
											</td>
			                                <td>
			                                ...
			                                <td>
			                                	<a href="#none" class="list-history" data-cpn_mst_idx="<c:out value="${row.CPN_MST_IDX }"/>">
		                                    		조회
	                                    		</a>
			                                </td>
			                            </tr>
...
$(function(){
    ...
    $(document).on("click", "#search", function () {
        ...
        if($searchForm.valid())
        {
	        ...
	        $searchForm.attr({"action":"<c:out value="${serverDomain}" />/am/prm/cpnMng/couponList.do", "target":"_self", "method":"post"}).submit();
        }
    });
    ...
    $(document).on("click", "#list-field .list-title", function () {
        ...
        if(viewType == "view")
        {
            url = "<c:out value="${serverDomain}" />/am/prm/cpnMng/couponEdit.do";
        }
 	...
 	// 발행내역 이동
    $(document).on("click", "#list-field .list-history", function () {
        var $frmPopup = $("#frmPopup");
        var $this = $(this);
        $frmPopup.find("input[name='CPN_MST_IDX']").val($this.data("cpn_mst_idx"));
        popup('',"1100","700","no","_searchProductPopup");
        $frmPopup.attr({"action":"<c:out value="${serverDomain}" />/am/prm/cpnMng/couponIssueHistoryPopup.do", "target":"popup_searchProductPopup", "method":"post"}).submit();
	...
	goForm = function(){
		var $frm = $("#frm");
		$frm.attr({"action":"<c:out value="${serverDomain}"/>/am/prm/cpnMng/couponForm.do", "target":"_self", "method":"post"}).submit();
	}
});
//-->
</script>
