	...
	<div class="modal">
		<div class="modal-wrapper">
			<div class="modal-inner">
					...
					<table cellspacing="0" class="table-row table-a" style="margin-bottom: 0;"><!-- table -->
						...
						<tbody>
							<tr>
								...
								<!-- 이벤트 완료여부 -->
								<th scope="col"><span>완료 여부</span></th>
								<td>
									<input type="radio" class="radio" name="evtEtrEdYn" id="evtEtrEdYn_" value="" checked="checked"/>
									<label for="evtEtrEdYn_">전체</label>
									<input type="radio" class="radio" name="evtEtrEdYn" id="evtEtrEdYn_Y" value="Y"/>
									<label for="evtEtrEdYn_Y">공개</label>
									<input type="radio" class="radio" name="evtEtrEdYn" id="evtEtrEdYn_N" value="N"/>
									<label for="evtEtrEdYn_N">비 공개</label>
								</td>
							</tr>

							<tr>
								<!-- 이벤트 완료일자 -->
								<th scope="col"><span>완료 기간</span></th>
								<td colspan="3">
				...
				<!-- 전체 참여 내역 영역 -->
				<div id="evtEtrMemHistList">
					<%-- <%@ include file="/WEB-INF/jsp/am/prm/evnMng/etrEvtMemHistPopupList.jsp" %> --%>
				</div>
				<!-- //전체 참여 내역 영역 -->
			</div>
		</div>
...
// 		console.log('frm',$frm.serialize());
		$("#popList").load("/am/prm/evnMng/etrEvtHistPopupList.do", $frm.serialize(), function(){
			// 전체 참여내역 초기화
			searchMemHist();
		});
	};
	...
	// 전체 참여 내역 조회
	searchMemHist = function(evtMstIdx, evtEtrRegId) {
		var $histfrm = $("#histfrm");
		console.log('$histfrm',$histfrm);

		$histfrm.find('input[name=evtMstIdx]').val(evtMstIdx);		// 이벤트 IDX
		$histfrm.find('input[name=evtEtrRegId]').val(evtEtrRegId);	// 참여자 ID (회원ID)

		$("#evtEtrMemHistList").load("/am/prm/evnMng/etrEvtMemHistPopupList.do", $histfrm.serialize(), function(){
		});
	}
	...
	excelDown = function() {
		var obj = {
				queryId: "Event.selectEtrEvtHistList",
				columnInfoFile: "etrEvtHistListExcel",
				fileName: "스템프형_이벤트_참여자내역",
				sheetName: "스템프형_이벤트_참여자내역",
