	                ...
	                <table cellspacing="0" class="table-row table-a"><!-- table -->
	                    ...
	                    <tbody>
	                        <tr><!-- row -->
	                            <th><span>구분</span></th>
	                            <td>
	                                <input type="radio" name="UPLOAD_GB" id="stateA" class="radio" value="Y" checked="checked"/>
		                            <label for="stateA">상품+이미지</label>
		                            <input type="radio" name="UPLOAD_GB" id="stateB" class="radio" value="N" />
                        ...
                        <tr><!-- row -->
                            ...
                            <td>
                                <c:if test="${not empty count }">
		                   			전체 업로드 상품수 : <c:out value="${count.ALL_CNT }"/>개
		                   		</c:if>
                            </td>
                        </tr>
                        <tr><!-- row -->
                            ...
                            <td>
                                <c:if test="${not empty count }">
		                   			<c:out value="${count.Y_CNT + count.P_CNT }"/>개
		                   			&nbsp;&nbsp;(이미지 등록 상품수 : <c:out value="${count.Y_CNT }"/>개)
		                   		</c:if>
                            </td>
                        </tr>
                        <tr><!-- row -->
			                        ...
			                        <c:if test="${not empty list }">
			                   	    	<c:forEach var="row" items="${list }" varStatus="i">
			                            	<tr>
						                    	...
						                    	<td><c:out value="${row.PRD_EXC_SEQ }"/></td>
					                            <td><c:out value="${row.PRD_EXC_NM }"/></td>
					                            <td><c:out value="${row.PRD_EXC_FAIL }"/></td> 
						               		</tr>
			                            </c:forEach>
			                         </c:if>
...
$(function(){
    ...
    $(document).on("click", "#goRegist", function () {
        ...
        var obj2 = $.trim($("#file2").val());
    	var pathpoint2 = obj2.lastIndexOf('.');
        var filepoint2 = obj2.substring(pathpoint2+1,obj2.length);
        ...
        if (obj2 != ""){
	        if ("zip" != filetype2){
	        	alert("["+filetype2+"] 형식은 업로드 할 수 없습니다.");
	        	return false;
	        }
        }
       	...
       	if (confirm( "저장 하시겠습니까?")) {
	        ...
	        $("#frm").attr({"action":"<c:out value="${serverDomain}" />/am/product/productUploadRegist.do", "target":"_self", "method":"post"}).submit();
	        $("#listGb").val("");
        }
