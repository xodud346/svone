		...
		<section id="contents"><!-- contents -->
			...
			<div class="contents-inner"><!-- contents-inner -->
                ...
                <form id="frm" name="frm" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="PRD_FILE_NAME" value="<c:out value="${commandMap.PRD_FILE_NAME }"/>" /><!-- 상품 이미지 파일명 - 팝업 등록에서 사용함 -->
                    <input type="hidden" id="pathType" name="pathType" value="product"/>
                    <input type="hidden" name="VDR_MST_IDX" id="VDR_MST_IDX" value="<c:out value="${commandMap.VDR_MST_IDX }" />" />
                    
                    <input type="hidden" name="PRD_MST_DLV_BND" value="0" /> <!-- 묶음배송여부 0 : no -->
						...
						<table class="core tdleft">
							...
							<tbody>
								<tr>
									...
									<td>
										<input type="text" id="PRD_MST_NM" name="PRD_MST_NM" class="text" style="width:70%" value="<c:out value="${productInfo.PRD_MST_NM }" />"  maxlength="150" disabled="disabled" />
										<br /><em class="strong">(공백포함 최대 100자이내로 입력해주세요.)</em>
									</td>
								</tr>
								<tr>
									<th><span>배송비 <em class="strong">*</em></span></th>
									<td><input type="text" id="PRD_MST_DLV_FEE" name="PRD_MST_DLV_FEE" class="number text short"  value="<fmt:formatNumber value="${productInfo.PRD_MST_DLV_FEE }" groupingUsed="true"  />" disabled="disabled"  maxlength="9"  /> 원</td>
									<th><span>상품명(서브)</span></th>
									<td>
										<input type="text" id="PRD_MST_NM_SUB" name="PRD_MST_NM_SUB" class="text" style="width:70%" value="<c:out value="${productInfo.PRD_MST_NM_SUB }" />"  disabled="disabled" maxlength="150"  />
										<br /><em class="strong">(공백포함 최대 100자이내로 입력해주세요.)</em>
									</td>
								</tr>
								<tr>
									<th><span>담당MD <em class="strong">*</em></span></th>
									<td><input type="hidden" name="PRD_MST_MD_ID" id="PRD_MST_MD_ID" value="<c:out value="${vendorManagerInfo.VDR_MST_MD_ID }" />" /> 
	                                    <c:if test="${not empty basicMdList }">
	                                        <c:forEach var="authRow" items="${basicMdList }" varStatus="i">
	                                        	<c:if test="${authRow.ADM_MST_ID eq vendorManagerInfo.VDR_MST_MD_ID}"><c:out value="${authRow.ADM_MST_NM }"/></c:if>
	                                        </c:forEach>
	                                    </c:if>
									</td>
									<th>비교검색여부/모델명</th>
									<td>
										<select name="PRD_MST_CPS_YN" id="PRD_MST_CPS_YN" class="select" disabled="disabled">
										...
										<input type="text" id="PRD_MST_CPS_MDL" name="PRD_MST_CPS_MDL" class="text" style="width:40%" value="<c:out value="${productInfo.PRD_MST_CPS_MDL }" />" disabled="disabled"  />
									</td>
								</tr>
								<tr>
								...
								<tr>			
									...
									<td>
										...
										<input type="text" id="PRD_MST_PC_CPN_PRC" name="PRD_MST_PC_CPN_PRC" class="number text short" value="<c:out value="${productInfo.PRD_MST_PC_CPN_PRC }" />"  maxlength=10 /><br />
										...
										<input type="text" id="PRD_MST_MB_CPN_PRC" name="PRD_MST_MB_CPN_PRC" class="number text short" value="<c:out value="${productInfo.PRD_MST_MB_CPN_PRC }" />"  maxlength=10 />
									</td>
								</tr>
<!-- 								<tr> -->
<!-- 									<th>묶음배송 여부</th> -->
<!-- 									<td> -->
<!-- 										<select name="PRD_MST_DLV_BND" id="PRD_MST_DLV_BND" class="select" disabled="disabled"> -->
<%-- 											<option value="0" <ui:select v="${productInfo.PRD_MST_DLV_BND}" t="0" g="selected"/>>NO</option> --%>
<%-- 											<option value="1" <ui:select v="${productInfo.PRD_MST_DLV_BND}" t="1" g="selected"/>>YES</option> --%>
<!-- 										</select> -->
<!-- 									</td> -->
<!-- 								</tr> -->
								<tr>
									<th>배송지정일 가능여부</th>
									<td>
										<select name="PRD_MST_DLV_DSN_YN" id="PRD_MST_DLV_DSN_YN" class="select" disabled="disabled">
										...
										배송 가능일 <input type="text" id="PRD_MST_DLV_DT" name="PRD_MST_DLV_DT" class="number text short" disabled="disabled" value="<c:out value="${productInfo.PRD_MST_DLV_DT}" />"  maxlength="9"  />&nbsp;일
									</td>
									<th>설치/사용 가이드</th>
									<td>
										<c:forEach items="${fileList}" var="fileRow" varStatus="i">	
											<c:if test="${fileRow.CMM_FLE_TB_TYPE == 'P' && fileRow.CMM_FLE_SORT == '1'}">
												...
												<input type="checkbox" class="checkbox cmmFleIdx" disabled="disabled" id="CMM_FLE_IDX_1" name="CMM_FLE_IDX" data-file_name="multiFile1" value="<c:out value="${fileRow.CMM_FLE_IDX }"/>" />
												<label for="CMM_FLE_IDX_1">삭제</label>
											</c:if>
										</c:forEach>
										<input type="file" name="file" id="multiFile1" disabled="disabled" class="file pcImgFile" style="width:50%;" data-attach_cnt="1" data-accept="img" />
									</td>
								</tr>	
								<tr>
									<th>등록일</th>
									...
									<th>수정일</th>
									<td>
										<c:if test="${productInfo.PRD_MST_UPD_NM }">
						...
						<table class="core tdleft">
							...
							<tbody>
								<tr>
									...
									<td><input type="text" id="PRD_MST_CPR_PRC1" name="PRD_MST_CPR_PRC1" class="number text" value="<fmt:formatNumber value="${productInfo.PRD_MST_CPR_PRC1 }" groupingUsed="true" />" disabled="disabled"  maxlength="9" /></td>
								</tr>
								<tr>
									...
									<td><input type="text" id="PRD_MST_CPR_PRC2" name="PRD_MST_CPR_PRC2" class="number text" value="<fmt:formatNumber value="${productInfo.PRD_MST_CPR_PRC2 }" groupingUsed="true" />" disabled="disabled"  maxlength="9" /></td>
								</tr>
								<tr>
									<th><span>패키지 할인가 </span></th>
									<td><input type="text" id="PRD_MST_PKG_PRC" name="PRD_MST_PKG_PRC" class="number text" value="<fmt:formatNumber value="${productInfo.PRD_MST_PKG_PRC }" groupingUsed="true" />" disabled="disabled" maxlength="9"  /></td>
									<th><span>매입원가<em class="strong">*</em> </span></th>
									<td><input type="text" id="PRD_MST_BUY_PRC" name="PRD_MST_BUY_PRC" class="number text" value="<fmt:formatNumber value="${productInfo.PRD_MST_BUY_PRC }" groupingUsed="true" />" disabled="disabled" maxlength="9"  /></td>
								</tr>
							</tbody>
						</table>
					</div><!-- // col list -->
						...
						<table class="core tdleft">
							...
							<tbody>
								<tr>
									...
									<td>
										<input type="checkbox" class="checkbox" name="PRD_MST_CNL_PC_YN" id="PRD_MST_CNL_PC_YN" value="Y" <ui:select v="${productInfo.PRD_MST_CNL_PC_YN }" t="Y" g="checked"/> disabled="disabled" /><label for="PRD_MST_CNL_PC_YN" > PC</label>
										<input type="checkbox" class="checkbox" name="PRD_MST_CNL_MB_YN" id="PRD_MST_CNL_MB_YN" value="Y" <ui:select v="${productInfo.PRD_MST_CNL_MB_YN }" t="Y" g="checked"/> disabled="disabled" /><label for="PRD_MST_CNL_MB_YN" > MOBILE</label>
									</td>
								...
								<tr>
									...
									<td><input type="hidden" id="PRD_MST_DPL_STATE" name="PRD_MST_DPL_STATE" value="<c:out value="${productInfo.PRD_MST_DPL_STATE }" />" />
										<c:if test="${productInfo.PRD_MST_DPL_STATE eq 'Y'  }">YES</c:if>
										<c:if test="${productInfo.PRD_MST_DPL_STATE eq 'N'  }">NO</c:if>
									</td>
									<th><span>승인상태</span></th>
									<td><input type="hidden" id="PRD_MST_APV_STATE" name="PRD_MST_APV_STATE" value="<c:out value="${productInfo.PRD_MST_APV_STATE }" />" />
										<c:if test="${productInfo.PRD_MST_APV_STATE eq 'D'  }">대기</c:if>
										<c:if test="${productInfo.PRD_MST_APV_STATE eq 'A'  }">승인</c:if>
								    </td>
								</tr>
								<tr id="pc_category">
										...
										<div id="divCategory"><!-- 카테고리 -->
											...
											<div class="table-a"><!-- col list -->
												<table class="core" id="tableCategory">
													...
													<thead>
														<tr>
															<th><input type="checkbox" class="checkbox checkAll_${Code.CATEGORY_TYPE_PC}" disabled="disabled"/></th>
															<th>카테고리 경로</th>
														</tr>
													</thead>
										...
										<div id="divCategory"><!-- 카테고리 -->
											...
											<div class="table-a"><!-- col list -->
												<table class="core" id="tableCategory">
													...
													<thead>
														<tr>
															<th><input type="checkbox" class="checkbox checkAll_${Code.CATEGORY_TYPE_MOBILE}" disabled="disabled" /></th>
															<th>카테고리 경로</th>
														</tr>
													</thead>
												...
												<c:set var="iconCnt" value="1" />
													<c:if test="${fn:length(Code.PRODUCT_ICON_NM) > 0 }">
														...
														<tr>
															...
															<td>
																<div class="field">
																	<input type="radio" id="PRD_MST_ICN_YN1_${iconCnt}" class="radio" name="PRD_MST_ICN_YN${iconCnt}" value="Y" disabled="disabled" <ui:select v="${PRD_MST_ICN_YN[iconCnt -1]}" t="Y" g="checked"/>  />
																	<label for="PRD_MST_ICN_YN1_${iconCnt}">사용</label>
																	<input type="radio" id="PRD_MST_ICN_YN2_${iconCnt}" class="radio" name="PRD_MST_ICN_YN${iconCnt}" value="N" disabled="disabled" <c:if test="${(PRD_MST_ICN_YN[iconCnt -1] eq null || PRD_MST_ICN_YN[iconCnt -1] eq '') || PRD_MST_ICN_YN[iconCnt -1] eq 'N'}" >checked=true</c:if> />
																	<label for="PRD_MST_ICN_YN2_${iconCnt}">미사용</label>
																</div></td>
															<td>노출기간 : 
																<input type="text" class="text" id="PRD_MST_ICN_ST_DT_${iconCnt}" name="PRD_MST_ICN_ST_DT${iconCnt}" disabled="disabled" value="<c:out value="${PRD_MST_ICN_ST_DT[iconCnt -1] }" />" />
																 ~
																<input type="text" class="text" id="PRD_MST_ICN_ED_DT_${iconCnt}" name="PRD_MST_ICN_ED_DT${iconCnt}" disabled="disabled" value="<c:out value="${PRD_MST_ICN_ED_DT[iconCnt -1] }" />" />
																(날짜형식: YYYY-MM-DD)
															</td>
															<td><input type="checkbox" id="PRD_MST_ICN_DT_YN_${iconCnt}" name="PRD_MST_ICN_DT_YN${iconCnt}" value="A" disabled="disabled" <c:if test="${PRD_MST_ICN_YN[iconCnt -1] eq 'Y' && (PRD_MST_ICN_ST_DT[iconCnt -1] eq '' && PRD_MST_ICN_ED_DT[iconCnt -1] eq '' )}">checked=true</c:if> /><label for="PRD_MST_ICN_DT_YN_${iconCnt}" > 제한없음</label></td>
														</tr>
																												...
																												<c:if test="${basicChnDtlList.CHN_MST_IDX eq basicChnMstList.CHN_MST_IDX }">
																													...
																													<tr>
																														...
																														<td style="border-top:0px;">
																															...
																															<input type="checkbox" disabled="disabled" name="PRD_CHN_PRC_YN_<c:out value="${basicChnMstList.CHN_MST_IDX }" />" id="PRD_CHN_PRC_YN_<c:out value="${basicChnMstList.CHN_MST_IDX }" />_<c:out value="${chnDtlIdx }" />" value="Y" class="checkbox" <ui:select v="${basicChnDtlList.PRD_CHN_DTL_YN}" t="Y" g="checked"/> /> <label for="PRD_CHN_PRC_YN_<c:out value="${basicChnMstList.CHN_MST_IDX }" />_<c:out value="${chnDtlIdx }" />"><c:out value="${basicChnDtlList.CHN_DTL_NM }" /></label>
																															<span style="padding-left:10px;">
																																<input type="hidden" name="OLD_PRD_CHN_PRC_<c:out value="${basicChnMstList.CHN_MST_IDX }" />" id="OLD_PRD_CHN_PRC_<c:out value="${basicChnMstList.CHN_MST_IDX }" />" value="<fmt:formatNumber value="${basicChnDtlList.PRD_CHN_PRC }" groupingUsed="true" />"   />
																																<input type="text" disabled="disabled" name="PRD_CHN_PRC_<c:out value="${basicChnMstList.CHN_MST_IDX }" />" id="PRD_CHN_PRC_<c:out value="${basicChnMstList.CHN_MST_IDX }" />" value="<fmt:formatNumber value="${basicChnDtlList.PRD_CHN_PRC }" groupingUsed="true" />" class="number text" <c:if test="${basicChnMstList.PRD_CHN_ECH_PRC eq 'N' || basicChnMstList.PRD_CHN_ECH_PRC eq null }" >disabled=true</c:if> maxlength="9" />
																															</span>
																														</td>
														...
														<td class="left">
															<table class="core">
																...
																<tbody id="applyChannelList">
																	<tr>
																		<td style="padding-left:10px;">
																			<select name="CHN_DEPT_1" id="CHN_DEPT_1" class="select" disabled="disabled">
																			...
																			<select name="CHN_DEPT_2" id="CHN_DEPT_2" class="select" disabled="disabled">
														...
														<c:choose>
															<c:when test="${not empty optionList}">
																...
																<input type="hidden" class="text" name="PRD_OPT_IDX" id="PRD_OPT_IDX" value ="<c:out value="${optionList.PRD_OPT_IDX}" />" />
						                                            <tr>  
																		<td><%-- <input type="button" name="goProductOptionDel" data-opt_idx="<c:out value="${optionList.PRD_OPT_IDX}" />" id="goProductOptionDel" value="-" /> --%></td>
																		<td>
																			<input type="text" class="text" disabled="disabled" name="PRD_OPT_NM" id="PRD_OPT_NM" style="width:80px;" value="<c:out value="${optionList.PRD_OPT_NM}" />" maxlength=200 />
																						...
																						<c:forEach var="optionDetailList" items="${optionDetailList }">
																							<c:if test="${optionList.PRD_OPT_IDX eq optionDetailList.PRD_OPT_IDX}">
																								<tr style="margin:0px; padding:0px;">
																									<td>
																										<input type="hidden" class="text" name="OPT_DTL_IDX" id="OPT_DTL_IDX" value ="<c:out value="${optionDetailList.OPT_DTL_IDX}" />" />
																										<input type="text" class="text" disabled="disabled" name="OPT_DTL_NM" id="OPT_DTL_NM" style="width:80px;" maxlength=30 value="<c:out value="${optionDetailList.OPT_DTL_NM }" />"/>
																									</td>
																									<td>
																										<input type="text" disabled="disabled" class="numbernr text" name="OPT_DTL_ADD_PRC" id="OPT_DTL_ADD_PRC" style="width:80px;" maxlength="9" value="<fmt:formatNumber value="${optionDetailList.OPT_DTL_ADD_PRC }" groupingUsed="true" />" />
																									</td>
																									<td>
																										<input type="text" disabled="disabled" class="number text" name="OPT_DTL_STK_CNT" id="OPT_DTL_STK_CNT" style="width:80px;" maxlength="9" value="<fmt:formatNumber value="${optionDetailList.OPT_DTL_STK_CNT }" groupingUsed="true" />" />
																									</td>
																									<td>
																										<input type="text" disabled="disabled" class="text datepicker s-datepicker" id="OPT_DTL_ST_DT_${detidx }_${subcnt}" name="OPT_DTL_ST_DT" value="<ui:formatDate value="${optionDetailList.OPT_DTL_ST_DT }" pattern="yyyy.MM.dd"/>" />
																										~
																										<input type="text" disabled="disabled" class="text datepicker e-datepicker" id="OPT_DTL_ED_DT_${detidx }_${subcnt}" name="OPT_DTL_ED_DT" value="<ui:formatDate value="${optionDetailList.OPT_DTL_ED_DT }" pattern="yyyy.MM.dd"/>" />
																									</td>
																									<td>
																										<select class="select" name="OPT_DTL_USE_YN" id="OPT_DTL_USE_YN" disabled="disabled">
																		...
																		<td>
																			필수여부&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
																			<select name="PRD_OPT_EST_YN" id="PRD_OPT_EST_YN" class="select" disabled="disabled">
																			...
																			<br />수량선택여부
																			<select name="PRD_OPT_CNT_YN" id="PRD_OPT_CNT_YN" class="select" disabled="disabled">
								...
								<div class="default"><!--  default  -->
									...
									<div class="clearfix">
										...
										<div class="fleft">
											...
											<p class="action">
												<!-- 슈퍼관리자만 수정 가능 -->
												<c:if test="${__MALL_ADMIN_SESSION_INFO__.AUT_MST_IDX eq Code.SUPER_ADMINISTRATOR_IDX || __MALL_ADMIN_SESSION_INFO__.AUT_MST_IDX eq Code.MD_ADMINISTRATOR_IDX}">	
...
<!--// 상품 이미지 등록 관련 코드 추가, 이상준, 끝 -->
					...
					<div class="section-form"><!-- section-form-a -->
						...
						<div class="form-group">
							...
							<div class="field">
								<input type="checkbox" class="checkbox" id = "detailChk" onclick="detailDesc();" disabled="disabled" />모바일에서 PC와 동일한 내용으로 사용<br />
								<textarea class="textarea" id="PRD_MST_DETAIL_DESC" name="PRD_MST_DETAIL_DESC" disabled="disabled" >${productInfo.PRD_MST_PC_DTL_DSC}</textarea>
<%-- 								<ui:editor objId="#PRD_MST_DETAIL_DESC"/> --%>
						...
						<div class="form-group">
							...
							<div class="field">
								<textarea class="textarea" id="PRD_MST_DETAIL_DESC2" name="PRD_MST_DETAIL_DESC2" disabled="disabled" >${productInfo.PRD_MST_MB_DTL_DSC}</textarea>
							</div>
						</div>
					</div><!-- // section-form-a -->
					</div>
						...
						<div class="table-a"><!-- col list -->
							...
							<tbody>
								<tr>
									<th><span>품목</span></th>
									<td colspan="3" class="left">
										<select class="select" name="PRD_MST_NFI_IDX" id="PRD_MST_NFI_IDX" disabled="disabled">
								...
								<tr>
									...
									<td class="left">
										...
										<input type="text" class="text" name="PRD_MST_BRD_NM" disabled="disabled" id="PRD_MST_BRD_NM" disabled="disabled" value="<c:out value="${productInfo.PRD_MST_BRAND_NM }" />" />
									</td>
									<th><span>제조사</span></th>
									<td class="left">
										<input type="hidden" class="text" name="PRD_MST_MRK_IDX" id="PRD_MST_MRK_IDX" value="<c:out value="${productInfo.PRD_MST_MRK_IDX }" />" />
										<input type="text" class="text" name="PRD_MST_MRK_NM" disabled="disabled" id="PRD_MST_MRK_NM" disabled="disabled" value="<c:out value="${productInfo.PRD_MST_MRK_NM }" />" />
									</td>
								</tr>
								<tr>
						...
						<div class="table-a"><!-- col list -->
							<table class="core tdleft">
								...
								<tbody>
									<tr>
										<th>메타태그1 : Author</th>
										<td><input type="text" id="PRD_MST_MTA_TAG1" name="PRD_MST_MTA_TAG1" class="text" disabled="disabled" style="width:60%" value="<c:out value="${productInfo.PRD_MST_MTA_TAG1 }" />"  /></td>
									</tr>
									<tr>
										<th>메타태그2 : Description</th>
										<td><input type="text" id="PRD_MST_MTA_TAG2" name="PRD_MST_MTA_TAG2" class="text" disabled="disabled" style="width:60%" value="<c:out value="${productInfo.PRD_MST_MTA_TAG2 }" />"  /></td>
									</tr>
									<tr>
										<th>메타태그3 : Keywords</th>
										<td>
											<input type="text" id="PRD_MST_MTA_TAG3" name="PRD_MST_MTA_TAG3" class="text" disabled="disabled" style="width:60%" value="<c:out value="${productInfo.PRD_MST_MTA_TAG3 }" />"   /><br />
						...
						<div class="table-a"><!-- col list -->
							<table class="core tdleft">
								...
								<tbody>
									<tr>
										<th>메모</th>
										<td><textarea class="textarea" name="PRD_MST_MEMO" id="PRD_MST_MEMO" style="height: 200px;" maxlength=2000 disabled="disabled"><c:out value="${productInfo.PRD_MST_MEMO }" /></textarea></td>
									</tr>
								</tbody>
							</table>
						</div><!-- // col list -->
				...
				<div class="section-button"><!-- section button -->
					...
					<a href="#none" class="button button-b" id="goRegist" ><span>수정요청</span></a>
...
<tr>
	<td class="center"><input type="checkbox" class="checkbox checkRow_\${PRD_CTG_ENV}" name="PRD_CTG_IDXs_\${PRD_CTG_ENV}" value="\${CTG_IDX}"/></td>
...
<tr>
	...
	<td class="left">
		...
		<input type="text" class="text"  disabled="disabled" style="width:60%;" name="PRD_MST_NFI_INF\${PRD_NFI_IDX}" id="PRD_MST_NFI_INF\${PRD_NFI_IDX}" value="\${PRD_MST_NFI_INF}"
		{{if PRD_NFI_INFO_YN =='Y'}}
			value="[자동입력]"
			disabled=true
		{{/if}} 
		/>
		<input type="hidden" name="B_PRD_NFI_INFO_CODE_\${PRD_NFI_INFO_CODE}" id="B_PRD_NFI_INFO_CODE_\${PRD_NFI_INFO_CODE}" value="\${PRD_NFI_INFO_CODE}" />
		<input type="hidden" name="A_PRD_NFI_INFO_CODE_\${PRD_NFI_IDX}" id="A_PRD_NFI_INFO_CODE_\${PRD_NFI_IDX}" value="\${PRD_NFI_INFO_CODE}" />
	</td>
</tr>
</script>
 ...
 // 상품아이콘 제한없음 클릭
    ...
    $(document).on("click", "#goRegist", function () 
    {
        ...
        if($("#PRD_MST_NM").val() == ""){
      	...
      	if($("#PRD_MST_DLV_FEE").val() < 0 || $("#PRD_MST_DLV_FEE").val() == ""){
      		alert("배송비를 입력해주세요.");
           ...
           if (confirm( "수정 하시겠습니까?")) 
           {
			...
			$("input[name=OPT_DTL_STK_CNT]").each(function(){
          	...
          	$("input[name^=PRD_CHN_PRC]").each(function(){
           	...
           	$frm.attr({"action":"<c:out value="${serverDomain}" />/am/product/basicProductVdrModify.do", "target":"_self", "method":"post"}).submit();
           } // confirm
    });    
