	                    	...
	                    	<c:choose>
			                	...
			                	<c:otherwise>
			                        ...
			                        <tr>
			                        	...
			                        	<td colspan=3>
			                        		<select class="select" name="searchVdrApvApvState" id="searchVdrApvApvState">
		                                        <option value="">전체</option>
		                                        <option value="D" <ui:select v="${commandMap.searchVdrApvApvState}" t="D" g="selected"/>>대기</option>
		                                        <option value="C" <ui:select v="${commandMap.searchVdrApvApvState}" t="C" g="selected"/>>거절</option>
		                                    </select>
			                        	</td>
			                        </tr>
			                        <tr>
	                ...
	                <div class="grid section-button-list"><!-- 목록 상단 버튼 -->
	                    <div class="col-1-2 text-left">
	                        ...
	                        <c:if test="${__MALL_ADMIN_SESSION_INFO__.AUT_MST_IDX eq Code.SCM_ADMINISTRATOR_IDX}">
	                            <a href="#none" id="productVdrModify" class="button small"><span>수정요청</span></a>
	                        </c:if>
	                	...
	                	<c:when test="${__MALL_ADMIN_SESSION_INFO__.AUT_MST_IDX ne Code.SCM_ADMINISTRATOR_IDX}">
			                ...
			                <thead>
			                    <tr>
                                    ...
                                    <th>검색가</th>
                                    <th colspan=2>배송비</th>
			                        <th rowspan="2">즉시할인쿠폰</th>			                        
			                        <th rowspan="2">입점업체</th>
			                        <th rowspan="2">등록일</th>
			                    </tr>
                                <tr>
				                    ...
				                    <c:when test="${not empty list}">
				                        ...
				                        <c:forEach var="item" items="${list }" varStatus="status">
				                            <tr>
				                                ...
				                                <td rowspan="2">
													<input type="text" class="text long" name="PRD_MST_NM_${item.PRD_MST_CD }" id ="PRD_MST_NM_${item.PRD_MST_CD }" value="<c:out value="${item.PRD_MST_NM}" />" maxlength="100"  />
													<br/><a href="javascript:goDetail('<c:out value="${item.PRD_MST_CD }" />');">(<c:out value="${item.PRD_MST_CD}"/>)</a>
				                                ...
				                                <td rowspan="2">
				                                    <ui:formatDate value="${item.PRD_MST_REG_DT }" pattern="yyyy.MM.dd"/>
				                                </td>
				                            </tr>
				                                            <td>
			                    ...
			                    <c:choose>
		                            <c:when test="${not empty list}">
		                                ...
		                                <tr>
		                                    ...
		                                    <td>
		                                        <input type="text" class="text" name="PRD_MST_NM_${item.PRD_MST_CD }" id ="PRD_MST_NM_${item.PRD_MST_CD }" value="<c:out value="${item.PRD_MST_NM}" />" maxlength="100" disabled="disabled" /><br/>
		                                        <%-- <a href="javascript:goPreview('<c:out value="${item.PRD_MST_CD }" />');">[→]</a> --%>
		                                    </td>
		                                    <td><c:out value="${item.PRD_MST_VDR_NM}" /></td>       
		                                    <td><input type="text" class="text short" name="PRD_MST_PRC_${item.PRD_MST_CD }" id="PRD_MST_PRC_${item.PRD_MST_CD }" value="<fmt:formatNumber value="${item.PRD_MST_PRC }" groupingUsed="true" />" /></td>
		                                    <td><input type="text" class="text short" name="PRD_MST_SCH_PRC_${item.PRD_MST_CD }" id="PRD_MST_SCH_PRC_${item.PRD_MST_CD }" value="<fmt:formatNumber value="${item.PRD_MST_SCH_PRC }" groupingUsed="true" />" /></td>
		                                    <td><input type="text" class="text short" name="PRD_MST_DLV_FEE_${item.PRD_MST_CD }" id="PRD_MST_DLV_FEE_${item.PRD_MST_CD }" value="<fmt:formatNumber value="${item.PRD_MST_DLV_FEE }" groupingUsed="true" />" disabled="disabled" /></td>
		                                    <td>
		                                        <select class="select" id="PRD_MST_DPL_STATE_${item.PRD_MST_CD }" name="PRD_MST_DPL_STATE_${item.PRD_MST_CD }" disabled="disabled" >
		                                            <option value="Y" <c:if test="${item.PRD_MST_DPL_STATE eq 'Y'}"> selected=true</c:if>>YES</option>
		                                            <option value="N" <c:if test="${item.PRD_MST_DPL_STATE eq 'N'}"> selected=true</c:if> >NO</option>
		                                        </select>
		                                    </td>
		                                    ...
		                                    <td><c:out value="${item.PRD_MST_APV_STATE_NM }" /></td>
		                                    <td><input type="text" class="text short" name="PRD_MST_SEL_CNT_${item.PRD_MST_CD }" id="PRD_MST_SEL_CNT_${item.PRD_MST_CD }" value="<fmt:formatNumber value="${item.PRD_MST_SEL_CNT }" groupingUsed="true" />" /></td>
		                                    <td>
		                                        PC
		                                        <select name="PRD_MST_PC_CPN_UNT_${item.PRD_MST_CD }" id="PRD_MST_PC_CPN_UNT_${item.PRD_MST_CD }" class="select">
...
<style>
.test, .short, .select { width: 100px !important; }
</style>
</body>
</html>
