		...
		<c:choose>
			<c:when test="${not empty list}">
				<c:forEach var="item" items="${list}" varStatus="status">
					<tr>
						<td>
							<input type="checkbox" id="CHK_DATA_ID_${item.EMP_NO}" name="CHK_DATA_IDs" class="checkbox checkRow"
								data-id="${item.EMP_NO}" data-nm="${item.OPRTR_NM}" />
			...
			<c:otherwise>
				<tr>
					<td class="td_no_result" colspan="8"><spring:message code="list.empty" /></td>
				</tr>
			</c:otherwise>
		</c:choose>
	</tbody>
