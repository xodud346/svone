	 ...
	 */
     function goStartMode(){
     		console.log("이미지 첨부 후 goStartMode");
     ...
     function goReadyMode(){
     	console.log("이미지 첨부 전 goReadyMode");
    	 var sSrc = welBtnConfirm.attr("src")|| "";
    	 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
    ...
    function html5Upload() {
    	...
    	sUploadURL= '/upload/SmartEditorHtml5Upload.do'; 	//upload URL
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		...
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/am/common/upload/SmartEditorUpload.do',	//샘플 URL입니다.
            sCallback : '/smartEditor/photo_uploader/popup/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 	    	...
 	    	select : function(oCustomEvent) {
				console.log('select',oCustomEvent);
 	    		//파일 선택이 완료되었을 때 발생
// 		    	 oCustomEvent (이벤트 객체) = {
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	success : function(oCustomEvent) {
 	    		console.log('success',oCustomEvent);
// 	    		alert("success");
 	    	...
 	    	error : function(oCustomEvent) {
				console.log('error',oCustomEvent);
				alert("error ");
 	    		//업로드가 실패했을 때 발생
 	    		//oCustomEvent(이벤트 객체) = {
 	    		...
 	    		console.log(oCustomEvent.htResult.errstr);
 	    	}
 	    });
 	}
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
 		console.log("setPhotoToEditor ");
 		
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			//스마트 에디터 플러그인을 통해서 넣는 방법 (oFileInfo는 Array)
			//img src 경로 임시 처리(2021-10-27 JAEYUN.JUNG)
			///oFileInfo[0].sFileURL = "/common/display.do?attachFileSavePath=" + oFileInfo[0].sFileURL;
			oFileInfo[0].sFileURL = "/nas" + oFileInfo[0].sFileURL;	//TODO 로컬을 위해 server docbase /nas 로 지정 개발서버에선 제거해야 함.
			console.log('setPhotoToEditor',oFileInfo);
			console.log("setPhotoToEditor========>" + setPhotoToEditor);
