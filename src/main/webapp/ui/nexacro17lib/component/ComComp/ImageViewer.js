...
if (!nexacro.ImageViewer) {
	...
	_pImageViewer.on_destroy_contents = function () {
		if (this._image) {
			...
			var image_height = this._image.width;
			...
			nexacro._releaseImageViewUrl(image_url, image_width, image_height, this.stretch);
	...
	_pImageViewer.on_apply_stretch = function () {
		...
		if (image) {
			...
			nexacro._releaseImageViewUrl(image_url, image_width, image_height, this._image.stretch);
	...
	_pImageAreaControl._on_load_image = function (imgurl, w, h) {
		...
		if (this.parent instanceof nexacro.ImageViewer) {
			...
			if (pos > 0) {
				...
				if (x != this.width || y != this.height || s != this.stretch) {
	...
	_pImageAreaControl._load_image = function () {
		...
		if (control_elem) {
			...
			if (v) {
				...
				if (this.parent instanceof nexacro.ImageViewer) {
					var image_size = nexacro._getImageViewSize(v, this._on_load_image, this, undefined, this.image, true, this.width, this.height, this.stretch);
				}
				else {
		...
		if (img_elem) {
			...
			if (!v || (w == 0 && h == 0)) {
				...
				if (this._getImageType() == "url") {
					...
					if (v != url) {
						bChange = false;
						nexacro._releaseImageViewUrl(url, this.width, this.height, this.stretch);
					}
				}
				...
				if (bChange) {
					...
					img_elem.setElementImage(url, w, h, this.stretch, true, this);
					this._updateElementPositions();
				}
			}
		}
