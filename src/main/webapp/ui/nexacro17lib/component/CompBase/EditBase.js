...
if (!nexacro.InputEventInfo) {
		...
		if (this.editmask != mask) {
			...
			for (var i = 0, len = mask.length; i < len; i++) {
				if (this.isMaskChar(mask[i])) {
					this.editmask_logical += mask[i];
					cnt_mask++;

					if (i == len - 1) {
						if (mask[i] == "y" && cnt_mask == 2) {
							this._cnt_editformat_year = cnt_mask;
						}
						else if (mask[i] == "M") {
							if (cnt_mask == 1) {
								this.editmask_logical += "M";
								this._cnt_editformat_month = cnt_mask;
							}
							else if (cnt_mask == 2 && this._cnt_editformat_month == 0) {
								this._cnt_editformat_month = cnt_mask;
							}
						}
						else if (mask[i] == "d") {
							if (cnt_mask == 1) {
								...
								this._cnt_editformat_day = cnt_mask;
							}
							else if (cnt_mask == 2 && this._cnt_editformat_day == 0) {
								this._cnt_editformat_day = cnt_mask;
							}
						}
					}
	...
	_pMaskTypeDate.isFilterCharEx = function (ch, pos_begin, pos_end) {
		...
		var _y;
		var h, h1, h2, m, m1, m2, s, s1, s2, ss;
		var editmask_type = this.getEditFormatType();
		var curr_mask = "";
		...
		if (editmask_type != 1) {
			...
			if (_msk == "M") {
				var _msk1 = this._cnt_editformat_month == 1 ? this.editmask_logical.substr(pos_begin + 1, 1) : this.editmask.substr(pos_begin + 1, 1);
				if (_msk1 == "M") {
					if (input_val > 1) {
