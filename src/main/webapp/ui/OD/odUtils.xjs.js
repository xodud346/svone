...
(function()
{
    return function(path)
    {
        ...
        this.fv_strCallback ="";
        this.fv_coCd = nexacro.getApplication().gds_userInfo.getColumn(0, "CO_CD");
        ...
        // 그리드 붙여넣기 Y/N
        this.fv_allowPaste = true;
        this.fn_callbackPaste;

        // 단축키 관련
        this.fv_shortCutKey = {ENTER:13, ESC:27, A:65, D:68, S:83};
        this.fv_shortCutMethod = {ENTER:"", ESC:"", SHIFT_ENTER:"", CTRL_ENTER:"", CTRL_A:"", SHIFT_D:"", SHIFT_S:""};
        ...
        this.copyPaste = {
        	...
        	allowPaste: function(allow, callback)
        	{
        		this.fv_allowPaste = allow;
        		this.fn_callbackPaste = callback;
        	},
        	...
        	pasteData: function(obj,copyData,e)
            {
        		...
        		for (var i = 0; i < rowDataCount; i++)
        		{
        			...
        			if( rowCount > currRow || (rowCount <= currRow && this.fv_grdAutoAddrow == true) )
        			{
        				...
        				{
        					if(i==0){
        						colid[j] = obj.getCellProperty("body", j, "text").substr(5);  //bind: 제거
        						colDpType[j] = obj.getCellProperty("body", j, "displaytype");
        					}
        					//ds.setColumn(currRow, colid[j], columnData[k]);
        					ds.setColumn(currRow, colid[j], ('number' == colDpType[j] && columnData[k] != undefined ? columnData[k].replaceAll(',','') : columnData[k]));
        					k++;
        				}
        				currRow++;
        			}
        ...
        this.fn_secureForIP = function(row, columnId, posIndex, dataset, delimiter, secureChar) {
        	...
        	//ipv6 단순 대응
        	if (ips.length == 1) {
        		var ips6 = orgValue.split(":");
        		if (ips6.length > 1) {
        			return this.fn_secureForIP(row, columnId, posIndex, dataset, ":");
        		}
        		else	return ips;
        	}
        	...
        	for (i = 0; i < ips.length; i++) {
        		if (i == posIndex || i == ((posIndex * 2) + 1))	rtnValue += secureResult;
        		else	rtnValue += ips[i];
        ...
        this.fn_regMethodENTER = function(method) {
        	this.fv_shortCutMethod.ENTER = method;
        	regShortCutMethod.call(this);
        };

        this.fn_regMethodESC = function(method) {
        	this.fv_shortCutMethod.ESC = method;
        	regShortCutMethod.call(this);
        };

        this.fn_regMethodShiftENTER = function(method) {
        	this.fv_shortCutMethod.SHIFT_ENTER = method;
        	regShortCutMethod.call(this);
        };

        this.fn_regMethodCtrlENTER = function(method) {
        	this.fv_shortCutMethod.CTRL_ENTER = method;
        	regShortCutMethod.call(this);
        };

        this.fn_regMethodCtrlA = function(method) {
        	this.fv_shortCutMethod.CTRL_A = method;
        	regShortCutMethod.call(this);
        };

        // fn_regMethodCtrlD -> fn_regMethodShiftD로 변경
        this.fn_regMethodShiftD = function(method) {
        	this.fv_shortCutMethod.SHIFT_D = method;
        	regShortCutMethod.call(this);
        };

        // fn_regMethodCtrlS -> fn_regMethodShiftS로 변경
        this.fn_regMethodShiftS = function(method) {
        	this.fv_shortCutMethod.SHIFT_S = method;
        	regShortCutMethod.call(this);
        };

        this.fn_regMethodSave = function(method) {
        	this.fv_shortCutMethod.SHIFT_S = method;
        	regShortCutMethod.call(this);
        };

        var regShortCutMethod = function() {
        	// 단축키
        	this.gfn_initFormHotKey({
        		ENTER : this.fv_shortCutMethod.ENTER,
        		ESC : this.fv_shortCutMethod.ESC,
        		CTRL_ENTER : this.fv_shortCutMethod.CTRL_ENTER,
        		CTRL_A : this.fv_shortCutMethod.CTRL_A,
        		SHIFT_ENTER : this.fv_shortCutMethod.SHIFT_ENTER,
        		SHIFT_S : this.fv_shortCutMethod.SHIFT_S,
        		SHIFT_A : "",
        		SHIFT_D : this.fv_shortCutMethod.SHIFT_D,
        	});
        };

        this.fn_processKey = function(obj,e) {
        	/*
        	 * Frame.js의 이벤트핸들러(onkeyup)와 중복으로 등록 되어, 두번 실행됨
        	 * regShortCutMethod를 통해서 이벤트핸들러를 한곳으로 모아서 처리함
        	 * 실제 이벤트 핸들러는 Frame.js의 FormHotKeyInstance
        	*/
        // 	if (e.keycode == this.fv_shortCutKey.ENTER && e.ctrlkey == false && e.shiftkey == false) {
        // 		if (this[this.fv_shortCutMethod.ENTER]) {
        // 			this.lookupFunc(this.fv_shortCutMethod.ENTER).call();
        // 		}
        // 	}
        // 	else if (e.keycode == this.fv_shortCutKey.ESC) {
        // 		if (this[this.fv_shortCutMethod.ESC]) {
        // 			this.lookupFunc(this.fv_shortCutMethod.ESC).call();
        // 		}
        // 	}
        // 	else if (e.keycode == this.fv_shortCutKey.ENTER && e.shiftkey == true) {
        // 		if (this[this.fv_shortCutMethod.SHIFT_ENTER]) {
        // 			this.lookupFunc(this.fv_shortCutMethod.SHIFT_ENTER).call();
        // 		}
        // 	}
        // 	else if (e.keycode == this.fv_shortCutKey.ENTER && e.ctrlkey == true) {
        // 		if (this[this.fv_shortCutMethod.CTRL_ENTER]) {
        // 			this.lookupFunc(this.fv_shortCutMethod.CTRL_ENTER).call();
        // 		}
        // 	}
        // 	else if (e.keycode == this.fv_shortCutKey.A && e.ctrlkey == true) {
        // 		if (this[this.fv_shortCutMethod.CTRL_A]) {
        // 			this.lookupFunc(this.fv_shortCutMethod.CTRL_A).call();
        // 		}
        // 	}
        // 	else if (e.keycode == this.fv_shortCutKey.D && e.shiftkey == true) {
        // 		if (this[this.fv_shortCutMethod.SHIFT_D]) {
        // 			this.lookupFunc(this.fv_shortCutMethod.SHIFT_D).call();
        // 		}
        // 	}
        // 	else if (e.keycode == this.fv_shortCutKey.S && e.shiftkey == true) {
        // 		if (this[this.fv_shortCutMethod.SHIFT_S]) {
        // 			this.lookupFunc(this.fv_shortCutMethod.SHIFT_S).call();
        // 		}
        // 	}
        };

        this.fn_getShortCutMethods = function() {
        	return this.fv_shortCutMethod;
        };
