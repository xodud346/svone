function getAddrLoc(){ //검색한 주소 데이터 가져오기
    document.formAPIDiv.keyword.value=validateJuso(document.formAPIDiv.keyword.value) ;  //특수문자 제거
    $.ajax({
         url :"/pc/common/getPostAddrApi.do"
        ,type:"post"
        ...
        ,success:function(xmlStr){
            ...
            if(errCode != "0"){
                ...
                if(xmlStr != null){
                    makeListDiv(xmlStr); //xml로 받아온 목록
                    pageDivMake(xmlStr); //totalCount로 페이징 처리
                }
            }
        }
