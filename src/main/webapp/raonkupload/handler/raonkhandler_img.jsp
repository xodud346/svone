...
<%	
	...
	final String rootPath = s.getString("file.path.root") + "/attachFile";
	
	//실제 업로드 할 기본경로를 물리적 경로로 설정합니다.
	upload.settingVo.setPhysicalPath(rootPath + "/rnd_image");

	//임시파일 물리적 경로설정 ( setPhysicalPath에 설정된 경로 + raonktemp )
	upload.settingVo.setTempPath(rootPath + strPathChar + "rnd_image_temp");

	// ***************보안 설정 : 업로드 가능한 경로 설정 - 이외의 경로로 업로드 불가능***************
	//String[] arrAllowUploadDirectoryPath = { rootPath + "/msds" };
	String[] arrAllowUploadDirectoryPath = { rootPath + "/rnd_image" };
	...
	String[] arrAllowDownloadDirectoryPath = { rootPath + "/rnd_image" , rootCnfg };
	...
	event.addAfterUploadEventListener(new AfterUploadEventListener() {
		public void afterUploadEvent(EventVo eventVo) {
			...
			try {
				...
				String sql = "{call SP_RD_PRD_MUTI_FILE_UPLD (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) }";
                ...
                cs.setString(4, customValue[1]);  //상품ID 필수
                cs.setString(5, customValue[2]);  //첨부파일유형코드-95:MSDS,S2:시약확인서 등등
                cs.setString(6, t[t.length - 1]);  //첨부파일명
                cs.setString(7, flength);  //첨부파일길이
                cs.setString(8, fpath);  //첨부파일저장경로
                cs.setString(9, fname);  //원본첨부파일명
                cs.setString(10, "SYSTEM");  //수정자 아이디

//              String sqlf = "{call SP_RD_PRD_MUTI_FILE_UPLD (%d, %d, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s') }";
