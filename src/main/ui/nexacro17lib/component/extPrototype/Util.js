 ...
 * @return {String} 문자열
*/
pForm.gfn_allTrim = function(sValue)
{
...
pForm.gfn_getLastDay = function(strDate)
{
	var s = "";
    if (strDate == null) {
	    var date = (new Date()).addMonth(1);
...
{	
    ...
    if (gfnGetMonthLastDate(strDate) == date.getDate()) {
    	...
    	var ld = new Date((new Date(d)).getFullYear() 
    			, (new Date(d)).getMonth()
    			, gfnGetMonthLastDate(this.gfn_dateToStr(new Date(d))));
...
/*
  투입 M/M 산정 함수
*/
pForm.gfn_getMonthPower = function(sStartDate, sEndDate){
	var sNextdate = "";
	var sLastdate = "";
	var nWorkDayCount =0;
	var nDayCount= 0;
	var nMenPower = 0.0;
	var nMonths = this.gfn_getDiffMonth(sStartDate,sEndDate)+1;
	var nReturn = 0;
	for(var i = 0 ; i < nMonths ; i++){
		//월에 시작일짜
		if(i == 0){
			sNextdate = sStartDate;
		} else {
			sNextdate = this.gfn_addMonth(sStartDate,i);
			sNextdate = this.gfn_getFirstDate(sNextdate);
		}
		
		//월의 마지막 일짜
		sLastdate = this.gfn_getLastDate(sNextdate);
		if(sEndDate < sLastdate){
			sLastdate = sEndDate;
		}
		
		nWorkDayCount = this.gfn_getDiffDate(sNextdate,sLastdate) + 1;
		nDayCount = this.gfn_getDiffDate(sNextdate.substr(0,6)+"01",this.gfn_getLastDate(sLastdate)) + 1;
		nMenPower = (nWorkDayCount*1.00)/nDayCount;
		
		nReturn = nReturn + nMenPower;
	}


	return nexacro.floor(nReturn,2);
};

/*
	이메일 유효성 검증
*/
pForm.validEmail = function(str) {
	var pattern = new RegExp(/^[a-z0-9A-Z._-]*@[a-z0-9A-Z]*.[a-zA-Z.]*$/);
...
/*
	휴대폰 번호 유효성 검증
*/
pForm.validMbNum = function(str) {
	var pattern = new RegExp(/^01(?:0|1|[6-9])-(?:\d{3}|\d{4})-\d{4}$/);
	return pattern.test(str);
};
...
/*
	전화번호 유효성 검증
*/
pForm.validTelNum = function(str) {
	var pattern = new RegExp(/^(02|0\d{2})-\d{3,4}-\d{4}$/);
	return pattern.test(str);
};